...
- (void)_drawBorder:(CGRect)rect unitSize:(CGSize)unitSize {
    ...
    if (_unitSpace < 2) {
        ...
        for (int i = (int)_characterArray.count; i < _inputUnitCount; i++) {
            ...
            CGContextAddPath(_ctx, bezierPath.CGPath);
             */
//            CGContextMoveToPoint( _ctx, unitRect.origin.x + (unitRect.size.width/4.f), unitRect.origin.y +unitRect.size.height );
//            CGContextAddLineToPoint( _ctx, unitRect.origin.x + (unitRect.size.width/4.f) + ((unitRect.size.width/4.f) * 2), unitRect.origin.y +unitRect.size.height );
            CGFloat tempW = (unitRect.size.width/10.f) * lineScale;
            CGFloat lineW = unitRect.size.width - (tempW * 2);
            CGContextMoveToPoint( _ctx, unitRect.origin.x + tempW, unitRect.origin.y +unitRect.size.height );
            CGContextAddLineToPoint( _ctx, unitRect.origin.x + tempW + lineW, unitRect.origin.y +unitRect.size.height );
        }
    }
...
- (void)_drawTrackBorder:(CGRect)rect unitSize:(CGSize)unitSize {
    ...
    for (int i = 0; i < _characterArray.count; i++) {
        ...
        CGContextAddPath(_ctx, bezierPath.CGPath);
         */
//        CGContextMoveToPoint( _ctx, unitRect.origin.x, unitRect.origin.y +unitRect.size.height );
//        CGContextAddLineToPoint( _ctx, unitRect.origin.x + unitRect.size.width, unitRect.origin.y +unitRect.size.height );
        CGFloat tempW = (unitRect.size.width/10.f) * lineScale;
        CGFloat lineW = unitRect.size.width - (tempW * 2);
        CGContextMoveToPoint( _ctx, unitRect.origin.x + tempW, unitRect.origin.y +unitRect.size.height );
        CGContextAddLineToPoint( _ctx, unitRect.origin.x + tempW + lineW, unitRect.origin.y +unitRect.size.height );
